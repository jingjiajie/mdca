...
from mdca.analyzer.ResultPath import ResultItem, ResultPath, CalculatedResult, BinResultValue
...
def chi2_filter(result: CalculatedResult, index: Index, search_mode: str) -> CalculatedResult | None:
    ...
    if len(filtered_items) == 0:
        return None
    elif all(map(lambda item: isinstance(item.value, BinResultValue), filtered_items)):
        return None
