...
from mdca.analyzer.MCTSTreeNode import MCTSTreeNode
    ...
    def __init__(self, data_index: Index, column_info: dict[str, ColumnInfo], target_column: str,
        ...
        for col in data_index.get_columns_after(None):
            if self.column_info[col].binning:
                continue
            self._categorical_column_values_candidate[col] = {}
            for val in data_index.get_values_by_categorical_column(col):
                loc: IndexLocations = data_index.get_categorical_locations(col, val)
                ...
                if data_index.target_column is not None:
                    ...
                    if target_loc.count < self.min_target_count:
                        continue
                self._categorical_column_values_candidate[col][val] = loc

    def _reset(self):
        root_loc: bitarray = bitarray(self.data_index.total_count)
        root_loc.setall(1)
        self._root = MCTSTreeNode(self, None, None, None, False, None, None, IndexLocations(root_loc), None)

    def _get_categorical_candidate_values_by_column(self, column: str) -> dict[Value, IndexLocations]:
        return self._categorical_column_values_candidate[column]
    ...
    def run(self, times: int):
        ...
        for i in range(0, times):
            if i != 0 and (i+1) % 1000 == 0:
                now: float = time.time()
                print(' - MCTS round: %d, cost: %.2f seconds' % (i+1, now - round_start))
                round_start = now
